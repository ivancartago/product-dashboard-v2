...
let monthlyCompareHeatmap = null;
...
function handleViewChange(e) {
    ...
    } else if (selectedView === "monthly-compare") {
        document.getElementById("year-select").disabled = true;
        document.getElementById("platform-select").disabled = false;
    }
...
function updateDashboard() {
    ...
    const monthlyCompareData = getMonthlyCompareData(currentProductData, selectedPlatform, hasEbooks);
    ...
    updateMonthlyCompareCharts(monthlyCompareData, hasEbooks, productLabel);
...
function updateViewVisibility() {
    ...
    const monthlyCompareView = document.getElementById("monthly-compare-view");
    
    // Hide all views
    yearlyView.classList.add("hidden");
    monthlyView.classList.add("hidden");
    platformView.classList.add("hidden");
    monthlyCompareView.classList.add("hidden");
    ...
    if (selectedView === "yearly") {
        ...
        monthlyCompareView.classList.remove("hidden");
        
        // Show appropriate message if no data
        const monthlyCompareEmptyMessage = document.getElementById("monthly-compare-empty-message");
        const monthlyCompareCharts = document.querySelectorAll("#monthly-compare-view .chart-wrapper");
        
        if (document.getElementById("monthly-compare-chart").getContext('2d').canvas.hasAttribute("data-has-data") === "false") {
            monthlyCompareEmptyMessage.classList.remove("hidden");
            monthlyCompareCharts.forEach(chart => chart.classList.add("hidden"));
        } else {
            monthlyCompareEmptyMessage.classList.add("hidden");
            monthlyCompareCharts.forEach(chart => chart.classList.remove("hidden"));
        }
    }
...
function updateChartTitles() {
    ...
    // Monthly Compare charts
    const monthlyCompareChartTitles = document.querySelectorAll("#monthly-compare-view .chart-title");
    monthlyCompareChartTitles.forEach(title => {
        if (title.textContent.includes("Monthly Comparison")) {
            title.textContent = `Monthly Comparison Across Years ${selectedPlatform !== 'All' ? `(${selectedPlatform})` : ''}`;
        } else if (title.textContent.includes("Monthly Performance")) {
            title.textContent = `Monthly Performance by Year ${selectedPlatform !== 'All' ? `(${selectedPlatform})` : ''}`;
        }
    });
...
// Update monthly compare charts
function updateMonthlyCompareCharts(monthlyCompareData, hasEbooks, productLabel) {
    const chartCtx = document.getElementById("monthly-compare-chart").getContext('2d');
    const heatmapCtx = document.getElementById("monthly-compare-heatmap").getContext('2d');
    
    // Set data-has-data attribute
    chartCtx.canvas.setAttribute("data-has-data", monthlyCompareData.hasData ? "true" : "false");
    
    // Don't update if no data
    if (!monthlyCompareData.hasData) {
        if (monthlyCompareChart) monthlyCompareChart.destroy();
        if (monthlyCompareHeatmap) monthlyCompareHeatmap.destroy();
        monthlyCompareChart = null;
        monthlyCompareHeatmap = null;
        return;
    }
    ...
    // Add datasets for each year
    Object.keys(monthlyCompareData.years).forEach(year => {
        // Skip years with no data
        if (!monthlyCompareData.years[year].hasData) return;
        
        compareData.datasets.push({
            label: year,
            data: MONTHS.map(month => {
                const monthData = monthlyCompareData.years[year].months[month];
                return monthData ? monthData.Total : 0;
            }),
            borderColor: yearColors[year],
            backgroundColor: yearColors[year] + '80', // 50% opacity
            tension: 0.1,
            fill: false
        });
    });
    
    // Prepare data for heatmap (we'll use a specialized bar chart)
    const heatmapData = {
        labels: MONTHS,
        datasets: []
    };
    
    // Create a dataset for each year in reverse chronological order
    Object.keys(monthlyCompareData.years)
    ...
    monthlyCompareChart = new Chart(chartCtx, {
        type: 'line',
        data: compareData,
        options: {
            responsive: true,
                ...
                tooltip: {
                    callbacks: {
                        ...
                        label: function(context) {
                            ...
                            return `${label}: ${value.toLocaleString()} sales`;
                        }
                    }
                }
            }
    ...
    // Create heatmap (stacked bar chart)
    monthlyCompareHeatmap = new Chart(heatmapCtx, {
        type: 'bar',
        data: heatmapData,
        options: {
            responsive: true,
...
// Helper function to get monthly comparison data across years
function getMonthlyCompareData(salesData, selectedPlatform, hasEbooks) {
    ...
    Object.keys(salesData.platforms).forEach(platform => {
        ...
        if (platformData.Physical) {
            ...
            Object.keys(platformData.Physical).forEach(year => {
                ...
                Object.keys(platformData.Physical[year]).forEach(month => {
                    const physicalValue = platformData.Physical[year][month] || 0;
                    ...
                    // Add physical data
                    result.years[year].months[month].Physical += physicalValue;
                    result.years[year].months[month].Total += physicalValue;
                    
                    // Mark as having data
                    if (physicalValue > 0) {
                        result.hasData = true;
                        result.years[year].hasData = true;
                    }
                });
            });
        }
        
        // Process eBook data if available
        if (hasEbooks && platformData.eBook) {
            // Get data for each year
            Object.keys(platformData.eBook).forEach(year => {
                ...
                // Process each month in this year
                Object.keys(platformData.eBook[year]).forEach(month => {
                    const eBookValue = platformData.eBook[year][month] || 0;
                    ...
                    // Add eBook data
                    result.years[year].months[month].eBook += eBookValue;
                    result.years[year].months[month].Total += eBookValue;
